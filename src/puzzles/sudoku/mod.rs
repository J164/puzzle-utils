...
pub const NUM_MIN: u8 = 1;
pub const NUM_MAX: u8 = 9;
...
fn solve(original: &Vec<u8>) -> Result<Vec<u8>, SudokuError> {
    ...
    if let Some(square) = next_blank(&sudoku, &mask, 0) {
        stack.push(square);
    } else {
        return Ok(sudoku);
    }
    ...
    while !stack.is_empty() {
        ...
        if let Some(square) = next_blank(&sudoku, &mask, *index) {
            stack.push(square);
        } else {
            return Ok(sudoku);
        }
...
fn next_blank(sudoku: &[u8], mask: &Mask, start: usize) -> Option<Square> {
    sudoku[start..]
        .iter()
        .enumerate()
