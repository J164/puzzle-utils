...
pub fn recursive_backtrack(width: u32, height: u32) -> Vec<Node> {
  ...
  let mut path = vec![0];
  let mut can_visit = vec![vec![Direction::Right,Direction::Down,Direction::Left,Direction::Up]; (width * height) as usize];
  while !path.is_empty() {
    let coordinate = path[path.len() - 1];

    match visit_next(coordinate, width, height, &mut maze, &mut connections, &mut can_visit[coordinate as usize]) {
...
fn visit_next(coordinate: u32, width: u32, height: u32, maze: &mut [Node], connections: &mut DisjointSet, visitable: &mut Vec<Direction>) -> Option<u32> {
  while !visitable.is_empty() {
    ...
    match rand_idx {
      Direction::Right => {
        ...
        if connections.find(coordinate as usize).unwrap() == connections.find(next as usize).unwrap() {
          continue;
        }
  
        maze[coordinate as usize].right = false;
        connections.union(coordinate as usize, next as usize).unwrap();
        return Some(next);
      } 
      Direction::Down => {
